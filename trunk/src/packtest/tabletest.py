    ...
    def addRow(self, pos, row):
        ...
        for item in row:
            ...
            if isinstance(item, Widget):
                ...
                e=Entry(self.spreadsheet, bg=colDat['bg'],fg=colDat['fg'], font=self.txtFont, justify=colDat['align'])
                e.internal=True
                e.insert(END, item)
...
if __name__ == '__main__':
    ...
    for i in range(0,20):
        ...
        ssw.addRow(END, (but, 'Important Message: %d' % i, 'John Doe','10/10/%04d' % (1900-i)))
